...
class DAG:
    ...
    def __repr__(self):
        def _repr(node: DAGNode, visited: set):
            ...
            visited.add(node)
            return (
                f"{node} -> [{', '.join(_repr(neighbor, visited) for neighbor in node.neighbors)}]"
                if node.neighbors
                else f"{node}"
            )
    ...
    def _dfs(
        self, node: DAGNode, predecessor: DAGNode, entered_nodes: set, left_nodes: set
    ):
...
def main():
    ...
    r = 3 * a**2
    r.backward(torch.tensor([1.0, 1.0]))
    print(a.grad)
