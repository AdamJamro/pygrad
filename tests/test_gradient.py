...
class TestValueBasicOperations:
    ...
    def test_matmul(self):
        A = Variable(numpy.array(
            [[1, 2],
             [3, 4]]
        ))
        x = Variable(numpy.array([5, 10]))
        Y = A @ x
        assert allclose(Y.value, numpy.array([25, 55]))
    ...
    def test_matmul_broadcast(self):
        ...
        x0 = numpy.random.rand(2)
        batch_size = 10
        x = numpy.array([x0 + i for i in range(batch_size)])
        A = Variable(A)
        x = Variable(x)
        Y = A @ x
